		...
		private double[,] GaussianFilter(int kernelSize, float sigma)
		{
			...
			for (int x = -center; x <= center; x++)
			{
				for (int y = -center; y <= center; y++)
				{
					int xDist = x - center;
					int yDist = y - center;
					gaussianKernel[x + center, y + center] = constant * Math.Exp(-(xDist * xDist + yDist * yDist) / (2 * sigma * sigma));
					sum += gaussianKernel[x + center, y + center];
					progressBar.PerformStep();								// Increment progress bar	
				}
			}
			progressBar.Visible = false;                                    // Hide progress bar
			...
			Console.WriteLine("Kernel values sum: " + sum);
